...
private:
    ...
    int _sockfd_server=-1; // handle to the server's socket
...
void CMoCapTCPServer<DataType_Send, DataType_Recv>::Stop()
{
    ...
    for(unsigned i = 0; i < _threadConnections.size(); i ++){
        ...
        if(iConnection >= 0){
            ...
            write(iConnection, (void*)&data, sizeof(data));
...
//            int a = ioctl(_sockfd_server, I_SETCLTIME, t);
        ...
        shutdown(_sockfd_server, SHUT_RDWR);
        int ret = close(_sockfd_server);    // wait until the write queue is clear
...
bool CMoCapTCPServer<DataType_Send, DataType_Recv>::InitializeServer()
{
    ...
    // use SO_REUSEADDR, so you may use the port again immediately after restarting the socket
    int optval = 1;
    if(setsockopt( _sockfd_server, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval) ) < 0){
                ...
                for(unsigned i = 0; i < _threadConnections.size(); i ++){
                    if(_threadConnections[i] != -1){ // 
                        ...
                        if(nbyte!=0){  //connection alive
                            auto n = send(_threadConnections[i], pDataBuffer, nTotSize,0);
                            if (n < 0)
                            ...
                            shutdown(_threadConnections[i], SHUT_RDWR);
                            close(_threadConnections[i]);
...
void CMoCapTCPServer<DataType_Send, DataType_Recv>::DoReceiveMessage(unsigned iClientThread)
{
    ...
    while(_bInWork){
        ...
        int n = recv(iConnection, pDataBuffer, nHeadSize,0);
